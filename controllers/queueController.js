...
module.exports = function ( app, ui) {
    ...
    // 4. post message to queue. input = queue URL and message // news

    app.post ('/mnsqueue-queue/message', async function (req, res) {

        var messagevalue = "{name:"+ req.body.name +",email:"+ req.body.email +",city:"+ req.body.city +",address:"+ req.body.address +",telephone:"+ req.body.zip +",country:"+ req.body.country +",total:"+ req.body.total +",state:"+ req.body.state +",id:"+ req.body.id +"}"
        req.body.name
        var qParams = {
            queuemessage: messagevalue,
            queueName: mns_config.queuename
            }   

        mnsq = await mns.sendMessage(qParams.queueName, {
            MessageBody: qParams.queuemessage
          });
